...
describe NumberGame do
  ...
  describe '#initialize' do
    context 'when solution is initialized' do
      ...
      matcher :be_between_zero_and_nine do
        match do |num|
          num.is_a?(Numeric) && num.between?(0, 9)
        end

        failure_message do |num|
          "expected #{num} to be betwin 0 and 9, but it wasn't"
        end

        failure_message_when_negated do |num|
          "expected #{num} not to be between 0 and 9, but it was"
        end
      end
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_over) { described_class.new(5, 2) }
      # remove the 'x' before running this test
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      let(:user_input) { '12' }
      let(:verify_input) { game_check.verify_input(user_input) }
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_loop).to receive(:player_input).and_return('12', '14', '7')
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      let(:game) { described_class.new(4, '4', 3) }
    ...
    context 'when count is 4 and over' do
      let(:game) { described_class.new(2, '2', 7) }
