...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user number is between arguments' do
      let(:min) { game_input.instance_variable_get(:@minimun) }
      ...
      # https://rubyapi.org/3.4/o/object#method-i-instance_variable_defined-3F

      it 'stops loop and does not display error message' do
        min = game_input.instance_variable_get(:@minimum)
        max = game_input.instance_variable_get(:@maximum)
        error_message = "Input error! Please enter a number between #{min}
 or #{max}."
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        invalid_input_one = 'ab'
        invalid_input_two = 'c2'
        valid_input = '7'
  ...
  describe '#verify_input' do
    ...
    let(:min) { binary_game.instance_variable_get(:@minimum) }
    let(:max) { binary_game.instance_variable_get(:@maximum) }

    context 'when given a valid input as argument' do
      before do
        valid_input = '7'
        allow(binary_game).to receive(:gets).and_return(valid_input)
      end

      it 'returns valid input' do
        expect(binary_game.player_input(min, max)).to eq(7)
      end
    end
    ...
    context 'when given invalid input as argument' do
      before do
        invalid_input = 'ab'
        valid_input = '5'
        allow(binary_game).to receive(:gets).and_return(invalid_input, valid_input)
      end

      it 'returns nil' do
        error_message = "Input error! Please enter a number between #{min} or #{
max}."
        expect(binary_game).to receive(:puts).with(error_message).once
        binary_game.player_input(min, max)
      end
    end
  end
