...
describe Drink do
  subject(:beverage) { Drink.new('juice', 16) }

  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      # Create an explicit subject, using 'described_class' and your choice of #beverage_type
      it 'is your choice of beverage' do
        expect(beverage.type).to eq('juice')
      end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      it 'is full' do
        expect(beverage.full?).to be true
      end
    end

    context 'when drink has less than 16 ounces' do
      # Create an explicit subject, using 'described_class' and your choice of #beverage type. In addition, specify ounces to be any number under 16.
      subject(:beverage) { Drink.new('ron', 8) }
      it 'is not full' do
        expect(beverage.ounces).to be <= 10
      end
    end
  end
end
