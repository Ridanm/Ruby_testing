...
describe FindNumber do
  ...
  describe '#game_over?' do
    let(:magic_number) { double('random_number', value: 5) }
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:end_play) { described_class.new(0, 9, magic_number, 5) }

      it 'is game over' do
        expect(end_play.game_over?).to be true
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:end_play) { described_class.new(0, 9, magic_number, 7) }

      it 'is not game over' do
        expect(end_play.game_over?).not_to be true
      end
    end
  end
