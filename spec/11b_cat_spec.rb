...
describe Cat do
  ...
  subject(:cat) { Cat.new('locky', 'siamese') }
  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when both classes share methods' do
    it_behaves_like 'pet'
  end

  # remove the 'x' before running this test
  it 'repond to hungry?' do
    expect(cat).to respond_to(:hungry?).with(0).arguments
  end
  ...
  # remove the 'x' before running this test
  it 'respomd to hiding' do
    expect(cat).to respond_to(:hiding?)
  end
