...
describe CaesarBreaker do
  ...
  describe '#create_decrypted_messages' do
    ...
    let(:fake_string) { ('a'..'z').to_a.map { |letter| "Abz#{letter}" } }

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends translate 26 times' do
      expect(phrase).to receive(:create_decrypted_messages).exactly(26).times
      fake_string.each do |msj|
        allow(translator).to receive(:message).and_return(msj)
  ...
  describe '#save_decrypted_messages' do
    ...
    context 'when the directory does not exist' do
      ...
      it 'sends message to create a file' do
        expect(File).to receive(:open).with('16_cipher/LippsAsvph.yaml', 'w').and_return(true)
        phrase.save_decrypted_messages
      end
    end
    ...
    context 'when the directory exists' do
      ...
      it 'does not send message to create a directory' do
        expect(Dir).not_to receive(:exist?).with('16_cipher')
      end

      it 'sends message to create a file' do
        expect(File).to receive(:open).with('16_cipher/LippsAsvph.yaml', 'w')
        phrase.save_decrypted_messages
      end
    end
  ...
  describe '#save_to_yaml' do
    ...
    it 'dumps to yaml' do
      message = phrase.instance_variable_get(:@message)
      expect(phrase).to receive(:puts).and_return("#{message} has been decode
d.")
      expect(phrase).to receive(:puts).and_return("The 25 possibilities are saved in a file in 16_cipher/#{message}")
      phrase.save_decrypted_messages
    end
  end
end
